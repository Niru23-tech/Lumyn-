...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let userId;
      if (isLogin) {
        userId = await mockLogin(email, password);
        if (!userId) throw new Error('Invalid credentials');
      } else {
        userId = await mockSignup(name, email, password, role);
        if (!userId) throw new Error('Could not create account');
      }
      
      await auth.login(userId);
      const user = await import('../services/mockApi').then(api => api.getMockUserById(userId));
