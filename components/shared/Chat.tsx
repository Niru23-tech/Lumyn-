...
import { getConversations, sendMessage as apiSendMessage } from '../../services/mockApi';
...
const Chat: React.FC<ChatProps> = ({ currentUser }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const receiver = selectedConversation.participant1.id === currentUser.id 
      ? selectedConversation.participant2 
      : selectedConversation.participant1;

    const sentMessage = await apiSendMessage(currentUser.id, receiver.id, newMessage);
      ...
      <div className="w-1/3 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.map(convo => {
            const otherUser = convo.participant1.id === currentUser.id ? convo.participant2 : convo.participant1;
            return (
              <div
                key={convo.id}
                onClick={() => handleSelectConversation(convo)}
                className={`p-4 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 border-b dark:border-gray-700 ${selectedConversation?.id === convo.id ? 'bg-gray-100 dark:bg-gray-700' : ''}`}
              >
                <div className="flex items-center">
                  <img src={otherUser.avatarUrl} alt={otherUser.name} className="w-10 h-10 rounded-full mr-3" />
                  <div>
                    <p className="font-semibold text-gray-800 dark:text-white">{otherUser.name}</p>
        ...
        {selectedConversation ? (
          <>
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center">
              <img src={ (selectedConversation.participant1.id === currentUser.id ? selectedConversation.participant2 : selectedConversation.participant1).avatarUrl } alt="avatar" className="w-10 h-10 rounded-full mr-3" />
              <h3 className="text-lg font-semibold">{ (selectedConversation.participant1.id === currentUser.id ? selectedConversation.participant2 : selectedConversation.participant1).name }</h3>
            </div>
